...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    $data = json_decode(file_get_contents("php://input")); /*Reauest etudiant json object // receiving data etudiant format json*/
    if (!empty($data->nom) && !empty($data->prenom) && !empty($data->age) && !empty($data->niveau_id)) {
        // On hydrate l'objet etudiant
        $etudiant->nom = htmlspecialchars($data->nom);    /*Conveertisseur*/
